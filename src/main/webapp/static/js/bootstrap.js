			...
			typeCheckConfig : function typeCheckConfig(componentName, config,
				...
				for ( var property in configTypes) {
					if (configTypes.hasOwnProperty(property)) {
						...
						var valueType = value && isElement(value) ? 'element'
								: toType(value);
	...
	var Button = function($) {
		...
		var Button = function() {
			...
			Button.prototype.toggle = function toggle() {
				...
				this._element.setAttribute('aria-pressed', !$(this._element)
			...
			Carousel._jQueryInterface = function _jQueryInterface(config) {
				return this
						.each(function() {
							...
							if ((typeof config === 'undefined' ? 'undefined'
							...
							if (typeof config === 'number') {
								...
								if (data[action] === undefined) {
									throw new Error('No method named "'
											+ action + '"');
								}
								data[action]();
	...
	var Collapse = function($) {
		...
		var Collapse = function() {
			...
			Collapse.prototype.hide = function hide() {
				...
				var dimension = this._getDimension();
				var offsetDimension = dimension === Dimension.WIDTH ? 'offsetWidth'
						: 'offsetHeight';

				this._element.style[dimension] = this._element[offsetDimension]
						+ 'px';
				...
				if (this._triggerArray.length) {
					$(this._triggerArray).addClass(ClassName.COLLAPSED).attr(
							'aria-expanded', false);
				}
			...
			Collapse.prototype._getParent = function _getParent() {
				...
				$(parent).find(selector).each(
						function(i, element) {
							_this8._addAriaAndCollapsedClass(Collapse
						...
						.each(function() {
							...
							var _config = $
									.extend(
											...
											Default,
											$this.data(),
											(typeof config === 'undefined' ? 'undefined'
													: _typeof(config)) === 'object'
													&& config);
	...
	var Dropdown = function($) {
		...
		var Dropdown = function() {
			...
			Dropdown._clearMenus = function _clearMenus(event) {
				...
				for (var i = 0; i < toggles.length; i++) {
					...
					if (event
							&& (event.type === 'click'
									&& /input|textarea/i
											.test(event.target.tagName) || event.type === 'focusin')
			...
			Dropdown._dataApiKeydownHandler = function _dataApiKeydownHandler(
				...
				if (!isActive && event.which !== ESCAPE_KEYCODE || isActive
		...
		$(document).on(Event.KEYDOWN_DATA_API, Selector.DATA_TOGGLE,
				...
				Selector.ROLE_MENU, Dropdown._dataApiKeydownHandler).on(
				Event.KEYDOWN_DATA_API, Selector.ROLE_LISTBOX,
				Dropdown._dataApiKeydownHandler).on(
				Event.CLICK_DATA_API + ' ' + Event.FOCUSIN_DATA_API,
				Dropdown._clearMenus).on(Event.CLICK_DATA_API,
				Selector.DATA_TOGGLE, Dropdown.prototype.toggle).on(
				Event.CLICK_DATA_API, Selector.FORM_CHILD, function(e) {
					e.stopPropagation();
				});
	...
	var Modal = function($) {
		...
		var Modal = function() {
			...
			Modal.prototype._setScrollbar = function _setScrollbar() {
				var bodyPadding = parseInt($(Selector.FIXED_CONTENT).css(
						'padding-right') || 0, 10);
		...
		var ScrollSpy = function() {
			...
			ScrollSpy._jQueryInterface = function _jQueryInterface(config) {
				return this
						.each(function() {
							var data = $(this).data(DATA_KEY);
							var _config = (typeof config === 'undefined' ? 'undefined'
									: _typeof(config)) === 'object'
									&& config;
	...
	var Tab = function($) {
		...
		var Tab = function() {
			...
			Tab.prototype.show = function show() {
				...
				if (listElement) {
					previous = $
							.makeArray($(listElement).find(Selector.ACTIVE));
			...
			Tab.prototype._activate = function _activate(element, container,
				...
				var isTransitioning = callback
						&& Util.supportsTransitionEnd()
						&& (active && $(active).hasClass(ClassName.FADE) || Boolean($(
								container).find(Selector.FADE_CHILD)[0]));

				var complete = function complete() {
					return _this21._transitionComplete(element, active,
			...
			Tab.prototype._transitionComplete = function _transitionComplete(
					element, active, isTransitioning, callback) {
				if (active) {
		...
		var Tooltip = function() {
			...
			Tooltip.prototype.show = function show() {
				...
				if (this.isWithContent() && this._isEnabled) {
					...
					$(tip).data(this.constructor.DATA_KEY, this).appendTo(
							container);
			...
			Tooltip.prototype.hide = function hide(callback) {
				...
				var complete = function complete() {
					if (_this23._hoverState !== HoverState.SHOW
							&& tip.parentNode) {
					...
					$(_this23.element)
							.trigger(_this23.constructor.Event.HIDDEN);
				...
				triggers
						.forEach(function(trigger) {
							if (trigger === 'click') {
								...
								var eventIn = trigger === Trigger.HOVER ? _this24.constructor.Event.MOUSEENTER
										: _this24.constructor.Event.FOCUSIN;
								var eventOut = trigger === Trigger.HOVER ? _this24.constructor.Event.MOUSELEAVE
							...
							$(_this24.element).closest('.modal').on(
			...
			Tooltip.prototype._fixTitle = function _fixTitle() {
				var titleType = _typeof(this.element
